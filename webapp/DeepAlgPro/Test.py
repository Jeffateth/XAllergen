...
def test(args):
    ...
    test_loader = DataLoader(dataset=test_ids, batch_size=args.batch_size, shuffle=True)
    model = convATTnet()
    model.to(device)
    ...
    with torch.no_grad():
        ...
        log(
            "Test Result: F1: "
            + str("%.5f" % f1_value.item())
            + "\tAccurcay: "
            + str("%.5f" % accuracy_value.item())
            + "\tPrecision: "
            + str("%.5f" % precision_value.item())
            + "\tRecall: "
            + str("%.5f" % recall_value.item())
            + "\tAUROC: "
            + str("%.5f" % auroc_value.item()),
            "./test.log",
        )
