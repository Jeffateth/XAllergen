...
def train(args):
    ...
    valid_loss_sum, accuracy_sum, recall_sum, precision_sum, auroc_sum, f1_sum = (
        0,
        0,
        0,
        0,
    ...
    for train_index, valid_index in skf.split(x):
        ...
        for epoch in range(args.epochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                log(
                    "[k: %d, batch: %d] train_loss: %.3f" % (k, i + 1, loss.item()),
                    "train.log",
                )
            ...
            (
                ...
                auroc_value,
                f1_value,
            ) = validation(model, x_valid, y_valid_label, criterion, args)
            ...
            log(
                ...
                "./model." + str(k) + ".fold.everyepoch.valid.txt",
            )
        ...
        log(
            "[k: %d] valid_loss: %.3f accuracy_value: %.6f recall_value: %.6f precision_value: %.6f auroc_value: %.6f f1_value: %.6f"
            % (
                k,
                valid_loss,
                accuracy_value,
    ...
    log(
        "valid_loss: %.3f accuracy_value: %.6f recall_value: %.6f precision_value: %.6f auroc_value: %.6f f1_value: %.6f"
        % (
            valid_loss_sum / 10,
            accuracy_sum / 10,
            recall_sum / 10,
