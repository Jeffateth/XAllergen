...
def main():
    ...
    argparser.add_argument('-i', '--inputs', default='./', type=str, help='input file (FASTA)')
    argparser.add_argument('-b', '--batch-size', default=1, type=int, metavar='N')
    argparser.add_argument('-o', '--output', default='allergenic_predict.txt', type=str, help='output file')
    argparser.add_argument('--save-attention', action='store_true', help='Save attention weights as .npy files')
    ...
    for record in SeqIO.parse(predict_fasta, 'fasta'):
        ...
        if length <= 1000:
            for _ in range(1, 1000 - length + 1):
                fastalist.append(0)
            for a in record.seq:
                fastalist.append(int(codeadict.get(a.upper(), "0")))  # fallback for unknown chars
